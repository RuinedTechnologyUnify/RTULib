...
public class JsonFile {
    ...
    public JsonFile(File file, JsonArray data, int savePeriod) {
        this.file = file;
        this.data = data;
        this.task = Bukkit.getScheduler().runTaskTimerAsynchronously(RTULib.getPlugin(), () -> {
            if (!needSave.get()) return;
            Bukkit.getScheduler().runTask(RTULib.getPlugin(), () -> {
                save();
                needSave.set(false);
            });
    ...
    protected boolean set(Pair<String, Object> find, Pair<String, Object> value) {
        ...
        if (value == null) {
            ...
            if (object instanceof JsonElement element) {
                ...
                RTULib.getPlugin().console(ComponentUtil.miniMessage("<red>Unsupported type of data tried to be saved! Only supports JsonElement, Number, Boolean, and String</red>"));
                RTULib.getPlugin().console(ComponentUtil.miniMessage("<red>지원하지 않는 타입의 데이터가 저장되려고 했습니다! JsonElement, Number, Boolean, String만 지원합니다</red>"));
                return false;
            }
    ...
    private void save() {
        try (Writer writer = new FileWriter(file)) {
            ...
            RTULib.getPlugin().console(ComponentUtil.miniMessage("<red>Error when saving " + file.getName() + "!</red>"));
            RTULib.getPlugin().console(ComponentUtil.miniMessage("<red> " + file.getName() + " 파일을 저장하는 도중 오류가 발생하였습니다!</red>"));
        }
    }
